...
{
    public class CloudSyncClientsPlugin : ICloudSyncClientsPlugin
    {
        private AsyncTimerService _timerService;
        public StoreCfgJson StoreCfgJson { get; set; } = new StoreCfgJson();
        public CloudSyncClientsDetector CloudSyncClientsDetector { get; set; } = new CloudSyncClientsDetector();
        public string pluginName => "CloudSyncClientsPlugin";
        ...
        public void Start()
        {
            ...
            {
                PluginContext.Log(pluginName, $"[CloudSyncClients] Unsupported triggerType '{triggerType}', plugin will not start.");
            }
        }
        ...
        {
            PluginContext.Log(pluginName, "[CloudSyncClients] interval hit");
            try
            {
                await Task.Run(() => CloudSyncClientsDetector.Instance.Check());
            }
            catch (Exception ex)
            {
