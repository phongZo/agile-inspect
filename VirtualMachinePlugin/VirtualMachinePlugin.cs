...
{
    public class VirtualMachinePlugin : IVirtualMachinePlugin
    {
        private AsyncTimerService _timerService;
        public StoreCfgJson StoreCfgJson { get; set; } = new StoreCfgJson();
        public VirtualMachineDetector VirtualMachineDetector { get; set; } = new VirtualMachineDetector();
        public string pluginName => "VirtualMachinePlugin";
        ...
        public void Start()
        {
            ...
            {
                PluginContext.Log(pluginName, $"[VirtualMachine] Unsupported triggerType '{triggerType}', plugin will not start.");
            }
        }
        ...
        {
            PluginContext.Log(pluginName, "[VirtualMachine] interval hit");
            try
            {
                await Task.Run(() => VirtualMachineDetector.Instance.Check());
            }
            catch (Exception ex)
            {
