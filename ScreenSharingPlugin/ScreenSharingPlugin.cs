...
{
    public class ScreenSharingPlugin : IScreenSharingPlugin
    {
        private AsyncTimerService _timerService;
        public StoreCfgJson StoreCfgJson { get; set; } = new StoreCfgJson();
        public ScreenSharingDetector ScreenSharingDetector { get; set; } = new ScreenSharingDetector();
        public string pluginName => "ScreenSharingPlugin";
        ...
        public void Start()
        {
            ...
            {
                PluginContext.Log(pluginName, $"[ScreenSharing] Unsupported triggerType '{triggerType}', plugin will not start.");
            }
        }
        ...
        {
            PluginContext.Log(pluginName, "[ScreenSharing] interval hit");
            try
            {
                await Task.Run(() => ScreenSharingDetector.Instance.Check());
            }
            catch (Exception ex)
            {
