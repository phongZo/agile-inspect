...
{
    public class SecureBootPlugin : ISecureBootPlugin
    {
        private AsyncTimerService _timerService;
        public StoreCfgJson StoreCfgJson { get; set; } = new StoreCfgJson();
        public SecureBootDetector SecureBootDetector { get; set; } = new SecureBootDetector();
        public string pluginName => "SecureBootPlugin";
        ...
        public void Start()
        {
            ...
            {
                PluginContext.Log(pluginName, $"[SecureBoot] Unsupported triggerType '{triggerType}', plugin will not start.");
                return;
            }
        }
        ...
        {
            PluginContext.Log(pluginName, "[SecureBoot] interval hit");
            try
            {
                await Task.Run(() => SecureBootDetector.Instance.Check());
            }
            catch (Exception ex)
            {
