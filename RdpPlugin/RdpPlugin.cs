...
namespace RdpPlugin
{
    public class RdpPlugin : IRdpPlugin
    {
        private AsyncTimerService _timerService;
        public StoreCfgJson StoreCfgJson { get; set; } = new StoreCfgJson();
        public RdpDetector RdpDetector { get; set; } = new RdpDetector();
        public string pluginName => "RdpPlugin";
        ...
        public void Start()
        {
            ...
            {
                PluginContext.Log(pluginName, $"[Rdp] Unsupported triggerType '{triggerType}', plugin will not start.");
                return;
            }
        }
        ...
        {
            PluginContext.Log(pluginName, "[Rdp] interval hit");
            try
            {
                await Task.Run(() => RdpDetector.Instance.Check());
            }
            catch (Exception ex)
            {
