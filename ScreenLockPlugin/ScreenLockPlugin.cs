...
{
    public class ScreenLockPlugin : IScreenLockPlugin
    {
        private AsyncTimerService _timerService;
        public StoreCfgJson StoreCfgJson { get; set; } = new StoreCfgJson();
        public ScreenLockDetector ScreenLockDetector { get; set; } = new ScreenLockDetector();
        public string pluginName => "ScreenLockPlugin";
        ...
        public void Start()
        {
            ...
            {
                PluginContext.Log(pluginName, $"[ScreenLock] Unsupported triggerType '{triggerType}', plugin will not start.");
                return;
            }
        }
        ...
        {
            PluginContext.Log(pluginName, "[ScreenLock] interval hit");
            try
            {
                await Task.Run(() => ScreenLockDetector.Instance.Check());
            }
            catch (Exception ex)
            {
