...
{
    public class DevModePlugin : IDevModePlugin
    {
        private AsyncTimerService _timerService;
        public StoreCfgJson StoreCfgJson { get; set; } = new StoreCfgJson();
        public DevModeDetector DevModeDetector { get; set; } = new DevModeDetector();
        public string pluginName => "DevModePlugin";
        ...
        public void Start()
        {
            ...
            {
                PluginContext.Log(pluginName, $"[DevMode] Unsupported triggerType '{triggerType}', plugin will not start.");
                return;
            }
        }
        ...
        {
            PluginContext.Log(pluginName, "[DevMode] interval hit");
            try
            {
                await Task.Run(() => DevModeDetector.Instance.Check());
            }
            catch (Exception ex)
            {
