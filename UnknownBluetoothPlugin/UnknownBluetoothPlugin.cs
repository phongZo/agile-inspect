...
{
    public class UnknownBluetoothPlugin : IUnknownBluetoothPlugin
    {
        private AsyncTimerService _timerService;
        public StoreCfgJson StoreCfgJson { get; set; } = new StoreCfgJson();
        public UnknownBluetoothDetector UnknownBluetoothDetector { get; set; } = new UnknownBluetoothDetector();
        public string pluginName => "UnknownBluetoothPlugin";
        ...
        public void Start()
        {
            ...
            {
                PluginContext.Log(pluginName, $"[UnknownBluetooth] Unsupported triggerType '{triggerType}', plugin will not start.");
            }
        }
        ...
        {
            PluginContext.Log(pluginName, "[UnknownBluetooth] interval hit");
            try
            {
                await Task.Run(() => UnknownBluetoothDetector.Instance.Check());
            }
            catch (Exception ex)
            {
