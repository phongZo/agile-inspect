...
{
    public class AutoUpdatePlugin : IAutoUpdatePlugin
    {
        private AsyncTimerService _timerService;
        public StoreCfgJson StoreCfgJson { get; set; } = new StoreCfgJson();
        public AutoUpdateDetector AutoUpdateDetector { get; set; } = new AutoUpdateDetector();
        public string pluginName => "AutoUpdatePlugin";
        ...
        public void Start()
        {
            ...
            {
                PluginContext.Log(pluginName, $"[AutoUpdate] Unsupported triggerType '{triggerType}', plugin will not start.");
                return;
            }
        }
        ...
        {
            PluginContext.Log(pluginName, "[AutoUpdate] interval hit");
            try
            {
                await Task.Run(() => AutoUpdateDetector.Instance.Check());
            }
            catch (Exception ex)
            {
